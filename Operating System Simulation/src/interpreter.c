...
int run(char* script){
	...
	copy_to_backing_store(script);
	if(errCode == 11){
		return handleError(errCode);
	}
	//run with FCFS
	initialize_process_from_backing_store(script);
...
int exec(char *fname1, char *fname2, char *fname3, char* policy, bool background, bool mt){	
	...
	if(fname1 != NULL){
		copy_to_backing_store(fname1);
        error_code = initialize_process_from_backing_store(fname1); //replace with backing store function?
		if(error_code != 0){
			return handleError(error_code);
		}
    }
    if(fname2 != NULL){
		copy_to_backing_store(fname2);
        error_code = initialize_process_from_backing_store(fname2);
		if(error_code != 0){
			return handleError(error_code);
		}
    }
    if(fname3 != NULL){
        copy_to_backing_store(fname3);
		error_code = initialize_process_from_backing_store(fname3);
		if(error_code != 0){
			return handleError(error_code);
		}
    } 
